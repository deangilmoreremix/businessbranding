...
interface MarketAnalysis {
  ...
  competitiveLandscape: {
    concentration: {
      cr4: number; // Concentration ratio of top 4 players
...
const MARKET_ANALYSIS_PROMPT = `Provide comprehensive market analysis for this industry:

Industry: {industry}
Brand Description: {brandDescription}

Please analyze:
1. Market size, growth rates, and projections
2. Key industry trends and their impact
3. Market segmentation and customer analysis
4. Competitive landscape and forces
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!GEMINI_API_KEY || GEMINI_API_KEY === 'demo-mode') {
      const demoResult: MarketAnalysis = {
        industry,
    ...
    console.log('Starting market analysis for:', industry);
    ...
    try {
      ...
      console.error('Failed to parse market analysis result:', parseError);
