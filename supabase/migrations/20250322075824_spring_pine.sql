/*
  # Fix tables setup issue for content management system

  This migration file revises the previous tables setup with correct syntax
  and ensures the tables are properly created.
...
CREATE POLICY "Allow full access to content_items" ON content_items FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "Allow full access to voice_profiles" ON voice_profiles FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "Allow full access to image_assets" ON image_assets FOR ALL USING (true) WITH CHECK (true);
